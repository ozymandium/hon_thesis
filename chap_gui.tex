...
When creating an application to run in realtime, it is of utmost importance that infomation be passed about in a prudent manner, or latency will accumulate and render the application  useless.
% had to use moos.
The MOOS framework \cite{moos} was already in place for communication between the DRTK, TDCP, and DAF algorithms, and the GPS receivers used.
% do more intro here.
...
Both GUIs were initially developed using postprocessed data collected from an Prowler ATV automously following an Infiniti G35---the original use of the DAF algorithm---using the same positioning and radio setup described in Sect. \ref{sec:hardware}. This was made possible by the playback feature of the MOOS framework, which is used to broadcast data from a text file as if it were streaming live.
